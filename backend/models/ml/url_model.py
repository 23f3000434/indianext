...
class UrlDetector:
    ...
    def _typosquatting_score(self, domain: str) -> float:
        ...
        Uses normalised Levenshtein distance.  A score near 1 means the
        domain is very close (but not identical) to a target brand.
        """
        best = 0.0
        for brand in self.TARGET_BRANDS:
            ...
            # Only flag when suspiciously close (similarity > 0.6)
            if similarity > 0.6:
